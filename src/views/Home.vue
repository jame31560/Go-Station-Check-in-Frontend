    ...
    >
      ...
      >
        <div v-if="infoContent">
          ...
          <p
            v-if="isLogin && infoContent.state == 1"
            class="text-center"
          >
            <a
              href="javascript:;"
              class="btn btn-success"
              v-on:click="checkin(infoContent, currentMidx)"
            >
              簽到
            </a>
          </p>
      ...
      checkin(infoContent, idx) {
        ...
        this.$http
          .post("http://192.168.1.102:5000/checkin", {
            gostation_id: infoContent.id
          })
          .then(
            res => {
              alert("簽到成功");
              infoContent.checked = true;
              this.markers.slice(idx, 1, infoContent);
            },
            err => {
